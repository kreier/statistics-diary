...
def check_year_file(folder, year):
    ...
    found_dates = set()
    bold_count = 0
    special_dates = []  # list of dicts {date, word_count}
    special_count = 0
    ...
    while i < len(lines):
        ...
        if match:
            ...
            if i > 0 and lines[i-1].strip().startswith("###"):
                ...
                special_dates.append({"date": date_str, "word_count": word_count})
...
def main():
    ...
    # Years to check
    years = [1976, 2021, 2022, 2023, 2024, 2025]
    ...
    for year, info in results.items():
        ...
        print(f"  Special dates (with ### above): {info['special_count']}")
    ...
    with open(csv_file, "w", newline="", encoding="utf-8") as f:
        ...
        writer.writerow([
            ...
            "Bold Count", "Special Count", "Missing Dates", "Special Dates"
        ])
        for year, info in results.items():
            # Format special dates as "date(wordcount)" joined by semicolon
            special_str = ";".join([f"{sd['date']}({sd['word_count']})" for sd in info["special_dates"]])
            writer.writerow([
                year,
                info["total_expected"],
                info["found"],
